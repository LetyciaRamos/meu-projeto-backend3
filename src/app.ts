...
app.post("/player/takeDamage", (req: Request, res: Response) => {
    ...
    const damageMessage = player1.takeDemage(damage);
    res.json({
...
app.listen(PORT, () => {
    ...
    console.log(`GET http://localhost:${PORT}/player- obter informaçoes do jogador`);
    console.log(`POST http://localhost:${PORT} /player/attack- jogador realiza um ataque`);
    console.log (`POST http://localhost:${PORT} /player/takedemage- jogador recebe dano`);
